...
router.post("/news", verifyUser, isAdminUser, async (req, res, next) => {
  try {
    ...
    if (updateStatus.stockStatus === true) {
      ...
      message =
        message + "Some of the requested stock tickers failed to update";
    }
...
router.get('/dashboard', verifyUser, isAdminUser, async (req, res, next) => {
    try {
        ...
        next(new customError("Error populating admin dashboard counts data", 500, "error"));
    }
});
...
router.get('/requests', verifyUser, isAdminUser, async (req, res, next) => {
    try {
        ...
        for (let i = 0; i < advisorsList.length; i++) {
            const request = commonUtils.formatUserDetails(advisorsList[0], "restricted");
...
router.post('/approve/:userId', verifyUser, isAdminUser, async (req, res, next) => {
    try {
