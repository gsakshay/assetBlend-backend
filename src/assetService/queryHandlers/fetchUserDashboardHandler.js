...
class FetchUserDashboardHandler {
    async handle(query){
        try{
            ...
            // get user assets:
            const fetchUserAssets = new FetchAssetList({user: userData._id, sold:false})
            ...
            if(userAssets.length > 0){
                ...
                const cryptoAssets = filteredUserAssets.filter(item => item.type === "crypto")
                const stockAssets = filteredUserAssets.filter(item => item.type !== "crypto")
                ...
                // Fetch stock result
                // handle exceptions
                dashboardData.stocks =  await Promise.all(stockAssets.map( async (stockData)=> {
                    let latestPrice = null
                    let profit=null
                    let dailyMetric = null
                    const processedStockData = stockData.toObject()
                    // get latest price
                    const latestStockData = await StockController.getPriceOnDate({}, stockData.ticker)
                    latestPrice = latestStockData[0]

                    // compute profit loss
                    profit = await computeProfitLoss(stockData.quantity,latestPrice.close, stockData.amountOnPurchase)
                    ...
                    try {
                        dailyMetric = await StockController.getDailyMetric(stock_req_params, stockData.ticker)
                    } catch (err) {
                        if (err.status === 400) {
                            dailyMetric = null
                        } else {
                ...
                try{
                    cryptoLatestPrices = await CryptoController.getPriceOnDate(cryptoRequestParams)
                }catch(e){
                    if(e.status === 400){
                ...
                if(cryptoLatestPrices.length > 0){
                    dashboardData.crypto = await Promise.all(cryptoAssets.map(async cryptoData => {
                        const processedCryptoData = cryptoData.toObject()
                        let latestPrice = null
                        let profit=null
                        let dailyMetric = null
                        const priceDataForCrypto = cryptoLatestPrices.find(latestPriceData => latestPriceData.ticker === cryptoData.ticker)
                        if(priceDataForCrypto && priceDataForCrypto.priceData && priceDataForCrypto.priceData.length > 0){
                            latestPrice = priceDataForCrypto.priceData[0]
                        }
                        // compute profit loss
                        profit = await computeProfitLoss(cryptoData.quantity,latestPrice.close, cryptoData.amountOnPurchase)
                        return {
            ...
            dashboardData.overAll_profitLoss = totalCurrentValue - userData.totalInvestedAmount
            // overall profit/loss %
            dashboardData.overAllPercent_profitLoss = null
            dashboardData.overAllPercent_profitLoss = (dashboardData.overAll_profitLoss/userData.totalInvestedAmount) * 100
            // return
            return dashboardData
        }catch(error){
            throw new customError("Failed to extract data for user dashboard", 500, 'error')
        }
    }
}
